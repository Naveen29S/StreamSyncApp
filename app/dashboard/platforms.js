...
const platforms = [
  ...
  { 
    ...
    description: 'Sync tweet impressions, followers count, engagement & recent posts' 
  },
  { 
    id: 'ig', 
    name: 'Instagram', 
...
export default function ConnectsScreen() {
  ...
  const [igClientId, setIgClientId] = useState('');
  const [igClientSecret, setIgClientSecret] = useState('');
  const [showIgAdvanced, setShowIgAdvanced] = useState(false);
  ...
  // Catch returning Instagram OAuth redirect (?code=...&state=instagram_oauth)
  useEffect(() => {
    async function checkInstagramOAuthCallback() {
      ...
      if (error && (state === 'instagram_oauth' || searchParams.get('connect') === 'ig')) {
        setOauthError(`Instagram authorization error: ${errorReason || error}`);
        window.history.replaceState({}, document.title, window.location.pathname);
        return;
      }

      if (code && (state === 'instagram_oauth' || searchParams.get('connect') === 'ig' || !searchParams.has('provider'))) {
        try {
          const { data: { session: currentSession } } = await supabase.auth.getSession();
          const userId = currentSession?.user?.id;
          if (!userId) {
            console.warn("Instagram OAuth callback detected, waiting for user session...");
            return;
          }
          ...
          const redirectUri = `${window.location.origin}/dashboard/platforms`;

          const result = await exchangeInstagramOAuthCode({
            code,
            redirectUri,
            userId,
          ...
          if (result.success) {
            ...
            setIgModalSuccess(`Successfully connected @${result.channel?.username || 'Instagram Creator'}!`);
            setTimeout(() => setIgModalSuccess(''), 4000);
          }
        } catch (exchangeErr) {
          console.error("Instagram OAuth exchange failed:", exchangeErr);
          setOauthError(exchangeErr.message || "Failed to connect Instagram account.");
        }
      }
    }

    checkInstagramOAuthCallback();
  }, []);
  ...
  async function handleDisconnectX() {
    setXLoading(true);
    try {
      setConnectedPlatforms(prev => prev.filter(p => !isPlatformMatch(p, 'x')));
      setXUsername('');
      setXBearerToken('');

      await disconnectPlatform('x');
      await fetchProfile(session.user.id);
      setXModalVisible(false);
    } catch (e) {
  ...
  function openIgModal() {
    const rawUser = apiKeys.ig_username || apiKeys.instagram_username || '';
    setIgUsername(rawUser);
    setIgModalError('');
    setIgModalSuccess('');
    setShowIgAdvanced(false);
    setIgModalVisible(true);
  }

  async function handleInstagramOAuthLogin() {
    setIgLoading(true);
    setIgModalError('');
    setIgModalSuccess('');
    try {
      ...
      if (typeof window !== 'undefined' && window.localStorage) {
        if (igClientId) localStorage.setItem('streamsync_ig_client_id', igClientId.trim());
        if (igClientSecret) localStorage.setItem('streamsync_ig_client_secret', igClientSecret.trim());
      }

      const { url } = getInstagramOAuthUrl({
        redirectUri,
        clientId: igClientId.trim() || undefined
      });

      setIgModalSuccess('Redirecting to official Instagram login...');
      setTimeout(() => {
        if (typeof window !== 'undefined' && window.location) {
  ...
  async function handleInstagramQuickSync() {
    ...
    try {
      ...
      setIgModalSuccess(`Successfully connected @${clean}!`);
      if (currentUserId) {
      ...
      setIgModalError(err.message || 'Failed to sync Instagram account.');
    } finally {
      setIgLoading(false);
    }
  }
  ...
  async function handleOAuthConnect(platformId) {
    ...
    switch (platformId) {
      ...
      case 'ig':
        setSyncing(null);
        await handleInstagramOAuthLogin();
        return;
      case 'in':
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bodyBg }]}>
            ...
            return (
              <View 
                ...
                style={[
                  ...
                  isDark && { borderWidth: 0, borderColor: 'transparent' },
                  isConnected && [styles.cardConnected, { borderColor: isDark ? 'transparent' : '#000', borderWidth: isDark ? 0 : 1 }]
                ]}
              >
      ...
      {/* ─── X (Twitter) Connect Modal ─── */}
      <Modal
        visible={xModalVisible}
        transparent={true}
        animationType="fade"
          ...
          <View style={[styles.modalContent, { backgroundColor: colors.cardBg, borderColor: colors.border, borderWidth: 1 }]}>
            ...
            <View style={{ 
              ...
              <Text style={{ fontSize: 12, fontWeight: '600', color: isDark ? '#f472b6' : '#be185d', flex: 1 }}>
                Instagram Login for Creators • Pure Instagram login without Facebook linking
              </Text>
            </View>
            ...
            <View style={styles.tabBody}>
              ...
              <TouchableOpacity 
                style={[
                  styles.googleOAuthBtn, 
                  { backgroundColor: '#E1306C', borderColor: '#E1306C', marginBottom: 6 }
                ]} 
                ...
                <View style={{
                  ...
                  <View style={styles.formGroup}>
                    ...
                    <TextInput
                      ...
                      onChangeText={setIgClientSecret}
                      secureTextEntry
                      autoCapitalize="none"
                    />
                  </View>
