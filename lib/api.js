...
export const fetchInstagramProfileData = async ({ username = 'creators', accessToken = '' } = {}) => {
  ...
  const lowKey = rawUser.toLowerCase().replace(/[@._]/g, '');
  const token = (accessToken || '').trim();

  // 1. Check verified influencer presets first
  ...
  // 2. If a valid live token is provided (not DEMO), attempt live Instagram Graph API
  if (token && token.toUpperCase() !== 'DEMO') {
    try {
      ...
      if (res.ok) {
        ...
        // Also attempt to fetch recent media / reels
        let fetchedVideos = [];
        try {
  ...
  // 3. Deterministic calibrated influencer engine for any custom handle
