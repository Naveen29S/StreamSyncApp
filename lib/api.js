...
export const fetchInstagramProfileData = async ({ username = '' } = {}) => {
  ...
  return generateInfluencerProfile(cleanUser);
};
...
export const fetchLinkedInProfileData = async ({ profileName = '', accessToken = '' } = {}) => {
  ...
  if (!rawProf && !token) {
    throw new Error('Please enter your LinkedIn Profile handle or company name.');
  }

  return {
    channel: {
      id: `in_${rawProf.toLowerCase().replace(/[^a-z0-9]/g, '') || 'profile'}`,
      username: rawProf ? `@${rawProf.replace(/^@/, '')}` : '@profile',
