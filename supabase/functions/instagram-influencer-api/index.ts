...
async function exchangeInstagramOAuth(params: {
  ...
  const videos = rawMedia.map((item: any, idx: number) => {
    ...
    const estViews = likes > 0 ? likes * 10 : Math.max(followers * 0.2, 500);
    ...
    return {
      ...
      likes,
      comments,
      engagement: estViews > 0 ? parseFloat((((likes + comments) / estViews) * 100).toFixed(1)) : 4.5,
  ...
  const calcTotalViews = impressions > 0 ? impressions : (rawMedia.length > 0 ? totalLikes * 12 : followers * 6);
  const engagementRate = calcTotalViews > 0 && totalInteractions > 0 
    ? parseFloat(((totalInteractions / calcTotalViews) * 100).toFixed(1)) 
    : 4.8;
  const estimatedRevenue = parseFloat(((calcTotalViews / 1000) * 1.75).toFixed(2));
  ...
  return {
    ...
    channel: {
      ...
      avatarUrl: profileJson.profile_picture_url || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=400&auto=format&fit=crop&q=80",
      totalFollowers: followers,
